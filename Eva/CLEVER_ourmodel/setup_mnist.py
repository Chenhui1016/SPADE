...
class mnist_0:
    def __init__(self, restore = None, session=None, use_softmax=False, use_brelu = False, activation = "relu"):
        ...
        model = Sequential()
        conv1 = Conv2D(32, (5,5), input_shape=(28,28,1), activation='relu', padding='SAME', use_bias=True)
        model.add(conv1)
        model.add(MaxPooling2D(pool_size=(2,2),padding='SAME'))
        conv2 = Conv2D(64, (5,5), activation='relu', padding='SAME', use_bias=True)                            
        model.add(conv2)
        model.add(MaxPooling2D(pool_size=(2,2),padding='SAME'))
        model.add(Flatten())
