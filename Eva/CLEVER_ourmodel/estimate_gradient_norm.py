...
class EstimateLipschitz(object):
    ...
    def load_model(self, dataset = "mnist", model_name = "2-layer", activation = "relu", model = None, batch_size = 0, compute_slope = False, order = 1):
        ...
        from setup_mnist import MNIST, mnist_0, mnist_0_download, mnist_01, mnist_02, mnist_03, mnist_03_download, TwoLayerMNISTModel
        ...
        if model is None:
            ...
            if dataset == "mnist":
                ...
                elif model_name == "mnist_0":
                    if activation == "relu":
                        model =  mnist_0("mymodels_mnist/mnist-0", self.sess, not output_logits)
                    else:
                ...
                elif model_name == "mnist_01":
                    if activation == "relu":
                        model =  mnist_01("mymodels_mnist/mnist-0.1", self.sess, not output_logits)
                    else:
                        print("actviation = {}".format(activation))
                        model = MNISTModel("models/mnist_cnn_7layer_"+activation, self.sess, not output_logits, activation= activation)
                        time.sleep(5)

                elif model_name == "mnist_02":
                    if activation == "relu":
                        model =  mnist_02("mymodels_mnist/mnist-0.2", self.sess, not output_logits)
                    else:
                        print("actviation = {}".format(activation))
                        model = MNISTModel("models/mnist_cnn_7layer_"+activation, self.sess, not output_logits, activation= activation)
                        time.sleep(5)

                elif model_name == "mnist_03":
                    if activation == "relu":
                        model =  mnist_03("mymodels_mnist/mnist-0.3", self.sess, not output_logits)
                    else:
                        print("actviation = {}".format(activation))
                        model = MNISTModel("models/mnist_cnn_7layer_"+activation, self.sess, not output_logits, activation= activation)
                        time.sleep(5)

                elif model_name == "mnist_0_download":
                    if activation == "relu":
                        model =  mnist_0_download("download/mnist-0", self.sess, not output_logits)
                    else:
                        print("actviation = {}".format(activation))
                        model = MNISTModel("models/mnist_cnn_7layer_"+activation, self.sess, not output_logits, activation= activation)
                        time.sleep(5)

                elif model_name == "mnist_03_download":
                    if activation == "relu":
                        model =  mnist_03_download("download/mnist-0.3", self.sess, not output_logits)
                    else:
                        print("actviation = {}".format(activation))
                        model = MNISTModel("models/mnist_cnn_7layer_"+activation, self.sess, not output_logits, activation= activation)
                        time.sleep(5)
