...
class CIFAR10Data(object):
    ...
    """
    def __init__(self, path, train_eps, eval_eps):
        ...
        self.train_data = DataSubset(train_images, train_labels, train_eps)
        self.eval_data = DataSubset(eval_images, eval_labels, eval_eps*np.ones((10000,1,1,1)))
...
class DataSubset(object):
    ...
    def get_next_batch(self, batch_size, multiple_passes=False, reshuffle_after_pass=True):
        ...
        if not multiple_passes:
            ...
            batch_eps = self.eps[self.cur_order[self.batch_start : batch_end], ...]
            self.batch_start += actual_batch_size
            return batch_xs, batch_ys, batch_eps
        actual_batch_size = min(batch_size, self.n - self.batch_start)
...
class AugmentedDataSubset(object):
    ...
    def get_next_batch(self, batch_size, multiple_passes=False, reshuffle_after_pass=True):
        ...
        return self.sess.run(self.augmented, feed_dict={self.x_input_placeholder:
                                                    raw_batch[0]}), raw_batch[1], raw_batch[2]
