...
config["num_steps"] = args.k
config["loss_func"] = args.loss

if args.method=="pgd":
    config["model_dir"] = "models/pgd_8.0"
elif args.method=="clean":
    config["model_dir"] = "models/pgd_0.0"
else:
    config["model_dir"] = "models/pgd-{}_6.0_8.0".format(args.method)
...
node_epsilon = np.ones((50000,1,1,1))
cifar = cifar10_input.CIFAR10Data(data_path, node_epsilon, 1.0)
...
def evaluate_checkpoint(filename):
  with tf.Session() as sess:
    ...
    for ibatch in range(num_batches):
      ...
      eps_batch = (args.eps * np.ones(bend-bstart)).reshape(-1,1,1,1)
